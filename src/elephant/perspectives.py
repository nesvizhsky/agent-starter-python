...
_agent: Agent[None, _Output] = Agent(
    ...
    system_prompt=(
        ...
        "set importance=3 rather than dropping it. NEVER create a story from generic "
        "reference material — encyclopedia entries, 'overview of X' or 'history of X' "
        "pages, topic hub/category index pages, or magazine issue front pages. These "
        "describe a topic in general, not something that happened — discard them even "
        "if no other article covers the same subject, rather than inventing an event "
        "or a date for them.\n"
