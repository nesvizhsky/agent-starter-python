...
def _digest(content: str, days_ago: int = 0) -> Digest:
    return Digest(
        id=uuid4(),
        topic_id=uuid4(),
        telegram_id=999,
...
def test_format_prompt_includes_topic_and_content() -> None:
    digests = [
        _digest("Ceasefire talks started Monday.", days_ago=3),
        _digest("Talks collapsed by Wednesday.", days_ago=1),
    ]
...
async def test_generate_produces_valid_synthesis() -> None:
    ...
    ), f"narrative_drift doesn't capture the week's arc: {result.narrative_drift[:200]}"
